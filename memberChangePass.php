...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($password !== $confirmPassword) {
        ...
        $query = "UPDATE tbl_members SET password = '$hashedPassword' WHERE id = $id";
        $result = mysqli_query($conn, $query);

        if ($result) {
           
            echo '<script>alert("Password Changed Successfull !!!."); window.location.href = "memberDashboard.php";</script>';
            exit();
        } else {
